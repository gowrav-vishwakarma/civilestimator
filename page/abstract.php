...
class page_abstract extends Page {
	function init(){
		parent::init();
		...
		$gs_m->addExpression('previous_qty')->set(function($m,$q)use($bill_m,$client_id,$bill_id,$project_id){
			...
			$prev_bill = $this->add('Model_Bill')
						->addCondition('order','<',(int)$bill_m['order'])
			...
			if($prev_bill !== null){
				$bdm->addCondition('order',$prev_bill);
			}else{
				$bdm->addCondition('order',-1);
			}
